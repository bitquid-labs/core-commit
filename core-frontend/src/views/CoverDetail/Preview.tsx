...
const Preview: React.FC<PreviewProps> = (props) => {
  ...
  const yearlyCost =
    ...
    const coverFees = parseFloat((parseFloat(coverAmount) * (yearlyCost / 100) * (coverPeriod / 365)).toFixed(14));
    const isCoverFeeTooSmall = coverFees < 0.000001;

  return (
    <div className="w-full h-full flex flex-col justify-between">
      <div className="relative flex flex-col gap-4 rounded-10 border border-[#FFFFFF33] bg-[#FFFFFF0D] md:px-25 md:py-50 px-20 py-28">
        <div className="flex flex-col gap-12">
          ...
          <div className="flex items-center justify-between">
            <div className="flex md:gap-[10px] gap-2">
              <div>Cover period</div>
              {/* <div className='bg-background-200 h-5 w-5 rounded-full' /> */}
            </div>
            <div className="font-semibold md:text-[16px] text-[12px]">
        ...
        <div className="flex items-center justify-between md:gap-0 gap-8">
...
<div>
              {isNaN(coverFees) || coverFees === 0 ? null : isCoverFeeTooSmall ? (
                ...
                <p className="md:text-lg text-m font-semibold text-emerald-400">{coverFees}</p>
              )}
            </div>

            <div className="md:ml-10 ml-5 rounded-6 bg-[#D9D9D933] md:px-[25px] md:py-[5px] px-5 py-2">{BQBTC.symbol}</div>
          </div>
        </div>
      </div>
